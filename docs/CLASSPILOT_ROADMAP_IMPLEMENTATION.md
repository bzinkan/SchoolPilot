...
- `pwsh -NoProfile -File tests/classpilot-runtime-config-deploy.test.ps1` passes 733 mocked deployment assertions, including the schema-v7 school pilot/off profiles, the schema-v9 school-scope unpin, preservation of unrelated capabilities, exact API/worker parity and rollback. No AWS mutation is part of that test.
