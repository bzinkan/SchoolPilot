...
- **Roadmap persistence**: Safety, scheduling, roster integration, website policy, content-category and monitoring-interruption schemas have additive checksum-ledger migrations mirrored in nonproduction convergence. `classpilotRoadmapPostExpand` is the 90-table target, not a live observation. CI exercises that target; Terraform's existing 75-table configuration stays unchanged pending the one-shot RLS admission and catalog verification. Use the exact `classpilotRoadmap` registry bundle for the first complete expansion; later feature-specific re-admissions must use their exact registered bundle.
