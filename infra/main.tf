...
locals {
  ...
  rls_post_expand_tables = local.rls_registry.inventories.mydeskImportsPostExpand.tables
...
check "rls_registry_contract" {
  assert {
    condition = (
      ...
      length(local.rls_post_expand_tables) == local.rls_registry.inventories.mydeskImportsPostExpand.count &&
