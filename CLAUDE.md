...
- **Coverage command scoping**: Claimed-student actions go through `POST /api/classpilot/coverage/contexts/:id/commands`, grouped by immutable supervision context. The coverage allowlist is `open-tab`, `close-tabs`, `lock-screen`, `unlock-screen`, `teacher-message`, `apply-flight-path`, `remove-flight-path`, `apply-block-list`, and `remove-block-list` (`COVERAGE_COMMAND_TYPES`). An ad hoc claim is no longer confined to it: claiming a student is the act of taking supervisory responsibility, so under the supervision-preview rollout a claimed context carries the same 15-command `SCHEDULED_CLASSROOM_COMMANDS` set a scheduled block does, adding `attention-mode`, `timer`, `poll`, `student-sign-out`, `temp-unblock`, and `limit-tabs`. Both origins resolve through the one predicate `scheduledContextHasClassroomTools`; the selected UI tab never establishes authority. Cross-context UI dispatch preserves partial outcomes and never converts one failed context into a class-wide fallback.
...
- **Semantic RLS registry**: `src/config/rlsRegistry.json` is the machine-readable inventory and rollout-request authority. It preserves the exact observed 72-table production snapshot from August 19, 2026 and separately defines the 75-table SchoolPilot 2.7.0 post-expand target. Production's 109-table inventory was verified on September 26, 2026 after My Desk reconciliation; `infra/production.tfvars` adopts its exact observed CSV order while the generic default remains at the separately staged 75-table target. Compare production and `mydeskImportsPostExpand` semantically; never rewrite historical inventory order or hashes to match a later observation. The earlier 90-table roadmap observation remains historical evidence. `classpilot_active_hands` remains part of the full inventory but is deliberately excluded from the four-table FAB re-admission bundle.
...
5. **Scheduler isolation** — Production web tasks run with `SCHEDULER_ENABLED=false`; the singleton ECS service `schoolpilot-production-scheduler-worker` runs `src/worker.ts` with `SCHEDULER_ENABLED=true`. Each scheduled job also takes a Postgres advisory lock through `runWithSchedulerLock()` so accidental duplicate workers do not double-run jobs. All heavy background jobs use `schedulerDb` from `src/services/schedulerDb.ts` (dedicated `pg.Pool`, `SCHEDULER_DB_POOL_MAX`, plus `SCHEDULER_LOCK_POOL_MAX`), isolated from the main API pool (`DB_POOL_MAX`). These are **caps, not defaults**: `databasePoolLimits()` resolves every pool as `Math.min(env, cap)` against the reviewed per-role profile in `src/config/databaseCapacity.json`, with `SCHEDULER_ENABLED` selecting the role — API `main` 16, worker `main` 2 / `scheduler` 5 / `schedulerLock` 8 — so a larger env value is silently clamped rather than honoured. An API process has no scheduler pool at all; `schedulerDatabaseAllowed()` throws on first use outside the worker or a migration task. Background jobs cannot starve API requests regardless of how long they take. When adding a new scheduled job, route it through `schedulerDb`, NOT the main `db` export. `schedulerDb` also sets `app.is_super='on'` on every connection, so it **bypasses Row-Level Security** — correct for cross-school jobs, but it means a scheduler query is NOT school-scoped (see "Database-Level Tenant Isolation (RLS)").

### ClassPilot Teacher Dashboard Commands
Teacher Dashboard actions are student-scoped, exact-binding, and outcome-driven:

- **Temporary supervision workflow**: `/classpilot/coverage?tab=live|scheduled|groups|access` is the Supervision hub; student monitoring and commands remain on the dashboard. Saved rosters and authorized staff do not establish live ownership. Ordinary claims use neutral Claimed students and preserve an existing context's deadline. Start, Send, and ad hoc end-time changes use `POST /api/classpilot/coverage/preview` followed by the existing mutation endpoint with its signed review token. The commit recomputes exact participants, authorization, source ownership, destination revision and lifetime under lifecycle/student locks; stale reviews fail without mutation. Reviewed starts never expand to the whole saved roster, and sends explicitly select a compatible unscheduled context or a new session. Scheduled reads project applied occurrences; scheduling shortcuts only prefill a draft. Retain legacy API compatibility, exact-binding screenshot transport, and owner/Observe separation. No extension release accompanies this dashboard/API flow.
...
- **Claimed-coverage route**: `POST /api/classpilot/coverage/contexts/:contextId/commands` accepts the coverage allowlist documented under School Isolation, widened to the full classroom set for any context that passes `scheduledContextHasClassroomTools` — scheduled or ad hoc alike. A claimed student therefore gets tiles, screenshots and the complete toolbar, one observation lease per claimed context so several groups preview at once. Outside the rollout the route still admits only the coverage allowlist.
- **Scheduled classroom authority**: `CLASSPILOT_SCHEDULED_CLASSROOM_MODE=off|on` defaults **on** for every school when unset; `off` is a global kill switch. `CLASSPILOT_SUPERVISION_PREVIEW_MODE=off|observe|on` likewise defaults **on**, and is the switch that governs an **ad hoc** claim — turning the scheduled mode off alone does not stop one. Carve a school out with `CLASSPILOT_SCHEDULED_CLASSROOM_EXCLUDED_SCHOOL_IDS` / `CLASSPILOT_SUPERVISION_PREVIEW_EXCLUDED_SCHOOL_IDS`; a malformed list refuses the rollout everywhere rather than guessing. The former allowlists `*_SCHOOL_IDS` are **retired and fail the boot**, because a leftover allowlist reads as "these schools have the feature" while every other school has it too. `GET /api/classpilot/dashboard-activity` is the personal scheduled workspace resolver. Class includes active teaching, scheduled testing/coverage, and deliberately started supervision. Ordinary claims carry `purpose: "claim"`, open Claimed, and never replace the current Class assignment; saved group membership is authorization/configuration, not an active testing purpose. `/commands` accepts exactly one `teachingSessionId` or `supervisionContextId`; expanded context tools are authorized only from the server-owned context row, through the single predicate `scheduledContextHasClassroomTools` — an active, in-window context carrying schedule application/coverage metadata needs the scheduled-classroom rollout, and one carrying none of it (an ad hoc claim) needs the supervision-preview rollout instead. `scheduledClassroomV1` plus `scopedAuthorityChecksV1` is required on the extension. Six classroom tables support exactly one same-school parent. Context settings, chat, hands, polls, Live View, observations and screenshots use that exact authority, with stale ownership rejected. No synthetic teaching sessions for testing. See `docs/CLASSPILOT_SCHEDULED_CLASSROOM_ACTIVATION.md` for the capability activation and its rollback, and `docs/CLASSPILOT_AUTOMATIC_CLASS.md` for the workspace behaviour.
- **Scheduled boundary worker**: `classpilot_school_schedules` carries an indexed deadline, generation and short lease, independent of the scheduling document revision. The one-second bounded worker reconciles lifecycle transitions; transaction wake triggers preserve concurrent edits. It has its **own** switch, `CLASSPILOT_SCHEDULE_BOUNDARY_WORKER_MODE=off|on`, which still defaults **off** and is deliberately not the scheduled-classroom mode above — that one now defaults on, while starting this worker drains a fleet-wide `next_boundary_at` backlog four schools at a time and stays an explicit decision. Keep report/backfill discovery outside this lane. Realtime plus known-boundary timers and reconnect checks target five seconds for healthy connected clients. Reporting-only classes remain awaiting live supervision, never active control.
...
- **Teacher FAB context**: render it for the teacher's owned active class **and for a supervision context they hold** — a scheduled block or a claimed context with `capabilities.fab === true`. Ordinary claims open the Claimed workspace; the selected context remains the authority for its tools. Label the context explicitly and close its panels when context changes. It remains hidden in Available and Observe-other modes.
...
- **Authoritative FAB state**: `fab-state-sync.data` carries the exact `studentId`/`studentSessionId`, ownership and lifecycle revisions, teaching session, active session IDs, toggle values, and active hands. Full state fans out on session start/end, settings changes, and coverage claim/release. A claimed student receives the **full** FAB — chat, raise-hand, polls and timers — whenever `scheduledContextHasClassroomTools` holds for their context and the device has accepted `scheduledClassroomV1` (`classpilotFab.ts`); the disabled/empty state now applies only outside the rollout or to a device that cannot negotiate. Stale ownership, session, or identity frames must still be rejected.
...
- **Coverage telemetry**: ownership transitions increment the control revision and fan out both classroom and FAB state. Assigned coverage staff and authorized administrators observing that exact context receive revision-bound tile telemetry. Observe subscriptions admit only sanitized monitoring, screenshot notices, and lifecycle invalidations; private chat, help, and response events remain owner-only. The admin-only `/api/classpilot/observable-activities` feed discovers current teaching/supervision authorities without granting mutation access. A delayed former-class update must fail the same locked ownership/binding check rather than leak after a claim.
...
- **Chrome extension release state**: The ClassPilot Chrome extension is MV3 and lives in the separate `C:\GitHub\ClassPilot` repository. The operator confirmed Chrome Web Store listing `iggbfegfcjkfieoemeolfmfnapepalca` is live at `2.7.2` on August 27, 2026. That proves public Store publication only; managed-Chromebook adoption and validation must not be inferred from the listing. Confirm the listing again immediately before uploading any successor. Use `console.warn` rather than routine `console.error` because Chrome surfaces the latter as visible extension errors to school IT.
- **Canonical packaging**: From a clean tagged ClassPilot commit in the ClassPilot repo root, run `./extension/package-extension.sh`. It reads the manifest, validates the packaged manifest version, and creates `dist/ClassPilot-v<manifest-version>.zip` plus compatibility copy `dist/classpilot-extension.zip`. Inspect the archive for root-level `manifest.json` and `managed_schema.json`, compare it byte-for-byte with source, and retain its SHA-256 before upload. A SchoolPilot deploy never publishes or updates the extension.
...
- **Protocol v3 repaired-client gate**: `scopedAuthorityChecksV1` is the required parent marker for all ten dependent repaired capabilities: `authBoundTelemetryV1`, `exactBindingAckV2`, `exactTabCloseV2`, `studentChatIdempotencyV1`, `screenshotObservationLeaseV1`, `screenshotTrackingWindowLeaseV1`, `screenshotActiveObservationCadenceV1`, `safetyEvidenceCaptureV1`, `liveViewIceServersV1`, and `kioskLaunchTicketV2`. The active-observation cadence additionally requires accepted `screenshotTrackingWindowLeaseV1`. The server must refuse every dependent unless the marker is advertised and accepted; never infer repair status from version text. Protocol-3 selected-tab controls require accepted `exactTabCloseV2`; protocol-2 clients retain negotiated V1 behavior. `kioskLaunchTicketV1` stays off. The 2.7.1 activation and rollback contract remains in `docs/CLASSPILOT_2_7_1_RELEASE.md`; the additive screenshot contracts are in `docs/CLASSPILOT_2_7_3_RELIABILITY_HARDENING.md` and `docs/CLASSPILOT_2_8_0_FAST_PREVIEW.md`.
- **MV3 lifetime rules**: The awake worker owns one scheduled 10-second heartbeat. Chrome's heartbeat alarm is recovery-only when that cadence becomes stale; it is not a second steady-state heartbeat source. Screenshots use an independent 30-second alarm. Negotiated 2.7.3 tracking-window capture is dashboard-independent; older clients retain their negotiated observation policy. Async offscreen/runtime messages must keep the MV3 event alive until ordered side effects, storage, and ACK work settle.
...
- **Managed shared-device continuity**: `managedDeviceContinuityV1` preflight requires accepted `scopedAuthorityChecksV1` plus `kioskLaunchTicketV2` before the extension reads an enterprise directory identifier. The issuance route immediately reduces that raw identifier through a ClassPilot-only school-scoped HMAC domain and returns a 10-minute audience-bound proof; raw directory IDs and recovery tokens must never be stored, returned, logged, or used as metric dimensions. The stateless proof is intentionally reusable for roster then login and bounded same-Chromebook handoffs, but every login still requires the selected student's PIN or email/ID credentials. Extension clients discard the proof after successful login. Other-device sessions remain hard blockers.
...
- **Screenshot pipeline**: `screenshotTrackingWindowLeaseV1` is an additive protocol-v3 capability, disabled by default and dependent on `scopedAuthorityChecksV1`. A negotiated 2.7.3 client captures with `chrome.tabs.captureVisibleTab` (JPEG quality 50, normally ~30–50 KB) about every 30 seconds throughout the authenticated, tracking-permitted window without requiring a dashboard. Each upload carries its actual pixel-capture time and exact server-granted authority. `student_session` uploads update only non-pixel capture-health evidence and discard the image body immediately. `teaching_session` uploads are revalidated under the exact live class, frozen roster, student session, device, control revision, entitlement, authentication, supervision, and deadline authority, then stored only under a class/revision-bound Redis key for 120 seconds. There is a third stored kind: a `supervision_context` upload is written to a supervision/revision-bound key under the same 120-second TTL and read back by the claim holder's tile batch. A device that has not negotiated `scheduledClassroomV1` has its supervision authority rewritten to `student_session` and stops capturing, which is byte-for-byte how that device behaved before the feature existed. Teacher reads return only that current class-bound key; class gaps and prior classes can never supply pixels. On the 30-second background cadence a current preview renders normally before 75 seconds, dims with an age label from 75 until 120 seconds, and is unavailable at 120. A tile on the observed `active_view` cadence uses `SCREENSHOT_ACTIVE_VIEW_STALE_MS = 15_000` instead, five times tighter, because frames are expected every five seconds; after #437 that applies to a claimed student's tiles as well. The extension cancels every capture phase when authority changes and immediately captures for the new authority. Authority identity is the exact claim plus the authenticated session/device binding, and there are **three** claim kinds: `{ kind: "student_session", controlRevision }`, `{ kind: "teaching_session", teachingSessionId, controlRevision }`, and `{ kind: "supervision_context", supervisionContextId, controlRevision }` — a supervision claim carries no `teachingSessionId`, and `parseClasspilotScreenshotAuthority` rejects any extra key; the `capturedAt` fence only rejects frames that precede the student session start, class start, or frozen-roster completion (30 s slow-clock tolerance) and is a race guard, not the identity check — control-state ACKs and unchanged re-pushes never move it. Existing 2.7.2 clients retain `screenshotObservationLeaseV1` behavior, and an exact-bound safety request remains independent. Teacher UI uses student-scoped batched `POST /api/classpilot/tiles/screenshots` and `/api/classpilot/tiles/history`; it never requests or exposes a device ID. `screenshotHealth` diagnostics remain public monitoring metadata on `/students-aggregated`.
- **Active preview cadence**: `screenshotActiveObservationCadenceV1` is separately negotiated and independently rolled out. Only an exact, current observation lease for that student may add `captureCadence: { mode: "active_view", intervalSeconds: 5, expiresInSeconds }` to the tracking-window policy — for a `teaching_session` authority the lease must match the same teaching session, and a `supervision_context` authority is served by its own per-context lease, so a claimed student reaches the five-second cadence too. Unobserved, mismatched, expired, student-session-only, or observation-store-unavailable cases return `background` at 30 seconds; they never fail open to five seconds. A capability-filtered exact-binding WebSocket nudge asks the extension to heartbeat when observation scope changes, but contains no device ID or policy grant and is not authoritative. The 120-second Redis pixel TTL and every existing screenshot authorization check remain unchanged. Activation, load gates, and rollback are in `docs/CLASSPILOT_2_8_0_FAST_PREVIEW.md`.
...
- **School domain is set at provisioning, never by self-registration**: a school's `domain` is set by a super admin (`POST /api/admin/schools`, `POST /api/schools`) and is what the extension uses to find the school. `POST /api/auth/register` is retired and answers `410 { code: "PUBLIC_REGISTRATION_RETIRED" }` with a request-independent body, recording the hit as a system audit event; prospects arrive through the public `POST /admin/school-inquiries` intake. It used to create an active school on the caller's unverified email domain, which could put a second school on a live school's domain and break student sign-in there.
- **Domain resolution rules**: `resolveSchoolForStudent` / `getSchoolsByDomain` ignore soft-deleted schools (a deleted school holds no domain claim) but keep suspended ones, which then fail at entitlement, so a suspended district sibling's students are never re-mapped onto the surviving school. Both lookups are ordered by `createdAt, id`: the oldest school and, on a shared domain, the oldest roster row win, so a later duplicate import cannot take over an already-rostered student.
...
- `PUBLIC_BASE_URL` — Canonical API origin. Managed-device continuity preflights and device proofs are signed for this normalized deployment audience and fail closed with a scoped 503 in production if it is absent or invalid; never derive this audience from request forwarding headers.
...
- `CLASSPILOT_PROTOCOL_V3_ENABLED` plus the per-capability flags are kill switches. `CLASSPILOT_CAPABILITY_ROLLOUTS_JSON` optionally supplies fail-closed `off`, `observe`, `canary`, or `on` policy per capability, with optional `schoolIds` and a deterministic school-level `canaryPercent`. Capability acceptance still requires a protocol-v3 client advertisement from the current exact binding, and all seventeen entries of `SCOPED_AUTHORITY_DEPENDENT_CAPABILITIES` require accepted `scopedAuthorityChecksV1`. `screenshotActiveObservationCadenceV1` also requires negotiated `screenshotTrackingWindowLeaseV1` and both `CLASSPILOT_CAP_SCREENSHOT_ACTIVE_OBSERVATION_CADENCE_V1=true` and a matching rollout entry. Use only guarded reviewed profiles: the schema-v5 `fast-preview-pilot` / `fast-preview-global-on` / `fast-preview-off` set documented in `docs/CLASSPILOT_2_8_0_FAST_PREVIEW.md`, the schema-v8 `scheduled-classroom-global-on` / `scheduled-classroom-off` family in `docs/CLASSPILOT_SCHEDULED_CLASSROOM_ACTIVATION.md`, which is the only sanctioned way to write the `scheduledClassroomV1` registry entry and deliberately has no pilot mode, and the schema-v9 `school-scope-unpin` profile in `docs/CLASSPILOT_SCHOOL_SCOPE_UNPIN.md`, which removes `schoolIds` from the fixed unpinnable set (`screenshotTrackingWindowLeaseV1`, `screenshotActiveObservationCadenceV1`, `studentAuthGatePresenceV1`) in one evidence-free apply and changes nothing else, while `restrictionAuthPassThroughV1` stays school-pinned by design; API and worker must receive the identical runtime registry. Superseded `kioskLaunchTicketV1` stays `off`. The approved synthetic-only exception records `validationLevel=synthetic_only` and `managedValidation=waived_not_passed`; it must never be described as managed-Chromebook validation.
...
GitHub Actions (`.github/workflows/ci-build.yml`) runs on push/PR to main. An app-only PR's critical path is ~11 min (`cross-tenant-tests`); a push to `main` always runs the full job set (~30 min) because `scripts/deploy.sh` gates on it and a deployed commit's SOC 2 packets must not record a skipped job.
- `changes` (pull requests only, `dorny/paths-filter@v3`): detects whether a PR touched any input of the ~30-minute `rollout-safety` job (`scripts/load/**`, `infra/**`, `scripts/deploy.sh`, the named PowerShell tools, `tests/**/*.ps1|psm1|py`, three docs it asserts on, and the workflow itself). `rollout-safety` is skipped on a PR only when that filter ran successfully AND proved no input changed; it fails closed (`!= 'false'`, `!cancelled()`) and always runs on `push`/`workflow_dispatch`. Never convert this to a workflow-level `on.*.paths` filter: `deploy.sh` refuses a commit with **zero** workflow runs, while a *skipped job* is green to it.
- Backend: `npm audit --audit-level=high` + `tsc --noEmit` + `npm run build`
- Frontend: `npm audit --audit-level=critical` + `npm run lint` + `vite build`, production-preview smoke, GoPilot/PassPilot builds, Android debug APKs and the dependency-audit chain (`audit:dependencies` collect → three post-build Router validations → `finalize`, which must stay in this one job, in order).
- `frontend-release-gates` (matrix, 3 shards): the 30 ClassPilot/kiosk release-focused suites, formerly serial inside `frontend`. Shard 1 = `test:staff-identity-contracts` + `test:classpilot-dashboard-load-state` (one file, serial tests, ~9 min — the floor); shard 2 = schedule-profiles + coverage-group-editor + coverage-workflow; shard 3 = the other 25 in their original order. Shard 3 holds every suite that spawns `vite --strictPort` on a fixed port (4177/4178/4179/4187/4188/4293) or uses Vite's default `node_modules/.vite` cacheDir, so shards must never share a runner. `test:release-focused` still exists for local use and must keep beginning with `npm run test:staff-identity-contracts` (`staff-identity-contracts.test.mjs` pins the literal). Every SOC 2 job that `needs: frontend` also needs this job, or the gates vanish from `CI_JOB_RESULTS`.
- Workflow-level `concurrency` cancels superseded **PR** runs only; every job has a `timeout-minutes`.
...
- **Safety alerts**: `sexual`, `violence`, `drugs`, `self-harm`, `weapons`, `hate`, `gambling`
- **Known lists**: `KNOWN_EDUCATIONAL` (Google, IXL, Khan Academy, first-party `nwea.org`/`mapnwea.org` and subdomains, etc.), `KNOWN_NON_EDUCATIONAL` (ESPN, YouTube, TikTok, etc.), `KNOWN_UNSAFE` (reviewed domain concerns → safety alert). NWEA assessment/auth/resource pages bypass the model; use hostname boundaries, not URL substring matching.
- **Search query detection**: Reviewed search-engine rules preserve explicit harm intent and method requests while bare suicide/self-harm topics, prevention hotlines and general educational/help-seeking queries do not trigger self-harm alerts. Match-local support/negation must not hide another explicit concern in the query.
- **Review first**: AI detections never close tabs or add website blocks. The shared Safety Center workflow notifies active school administrators; administrators choose review, exact-URL approval, or explicit school website blocking.
- **Allowed domains**: `allowedDomains` and Flight Paths affect off-task classification, not safety approvals. Safety Center exact-URL approvals suppress subsequent safety alerts only; independent website blocks remain enforced.
- **Notifications**: Repeated observations merge by open student case, exact URL and concern. Each distinct alert receives one initial administrator notification and one unacknowledged follow-up; queued report alerts are bundled. There is no recurring reminder loop.
...
- **Administrator review contract**: Safety Center is an Admin panel card below Coverage and above Database Cleanup, at `/classpilot/admin/safety`. `safetyCenter.ts` owns one open school/student case, distinct alerts, private administrator events, encrypted exact-URL approvals and their revocation. Browser and enabled MailPilot detections use `safetyNotifications.ts` for durable mail to active `admin`/`school_admin` memberships, one initial per distinct alert and one unacknowledged follow-up. Severity never gates the initial mail. Legacy safety-email/automatic-close settings must not override this contract. AI classifications never issue closure commands. URL approvals suppress safety detection only; explicit website policies and teacher authority remain independent. See `docs/CLASSPILOT_SAFETY_CENTER.md`.
- **Retention boundary**: Keep minimal open-case provenance and merged-case redirects while the canonical case is retained. Raw URL ciphertext and legacy URL summaries follow the school browsing retention window independently of the case-spine count/delete switch. Existing email-content and screenshot retention remain separate. Active URL approvals are encrypted configuration until revoked; their optional source-alert reference is detached when that source expires. Never rewrite previously exported packet bytes.
- **Roadmap persistence and release**: The eight additive checksum-ledger roadmap migrations and SchoolPilot backend/web release `5a2d365b3b3184248f43e75ff572d05cd50df51a` deployed on September 6, 2026. Live verification at `2026-09-06T04:07:10.65Z` found all 90 expected tables with enabled and forced RLS, the `tenant_isolation` policy, and a matching runtime allowlist. API `schoolpilot-production-api-emergency:109` and worker `schoolpilot-production-scheduler-worker:125` reached strict stability with one task each and the prior scaling state restored. The separate production baseline adoption copies the observed CSV; no Terraform apply was performed. Preserve the registry's historical 72-table observation and 75-table target. Omit the one-shot flag on later deploys; reversing a deliberate kill-switch still requires the exact registered re-admission bundle. Operator evidence is retained under the ignored `soc2-evidence/releases/2026-09-05-roadmap/` directory, not in tracked public documentation. See `docs/CLASSPILOT_ROADMAP_IMPLEMENTATION.md` for release identities and remaining feature/extension gates.
- **Reusable schedule profiles**: Class Management → Scheduling supports named templates and separately previewed dated applications for fixed-time/period classes plus optional Coverage testing groups. `classpilotScheduleProfiles.ts` owns revisioned admin mutations; the general scheduling writer must not edit or erase template/application collections. Applied snapshots preserve regular rosters and only alter eligible dates. Cancelling a whole application still requires acting before the first affected original/proposed start (it 409s with `SCHEDULE_APPLICATION_STARTED` afterwards), but a **single date/block pair can be cancelled after it has started** via `cancelledBlocks`, and its students fall back to their regular bell-schedule class. Applying a profile onto a date that already carries one returns advisory `warnings` and 409s with `SCHEDULE_EXISTING_APPLICATIONS` unless `acknowledgeExistingApplications` is sent. `classpilotScheduleProfileValidation.ts` checks full monitoring windows and frozen/approved/current teaching obligations at preview, apply and activation. The worker uses exact frozen group membership, current claim/manage pairings and existing supervision authority, records terminal outcomes, and never restarts an early-released context. Apply `classpilot-schedule-profile-supervision-20260905` before API/worker activation; it adds metadata to existing registered tenant tables without changing the RLS table count. See `docs/CLASSPILOT_SCHEDULING.md`.
...
- Date-selected browsing uses the separate `/classpilot/students/:studentId/browsing-history` cursor API. Teacher authority comes from frozen historical staff/roster and supervision windows; whole-day totals remain administrator-only. Partial supervision cannot expose a whole-session aggregate. See `docs/CLASSPILOT_BROWSING_HISTORY.md`.
...
- `settings.passpilot_class_source` controls manual PassPilot class selection and roster management: `legacy_grades` preserves standalone PassPilot behavior, while `classpilot_groups` uses active official ClassPilot `admin_class` groups, `group_students`, and primary/co-teacher assignments. A teacher may explicitly opt their kiosks into `classpilot` scheduling independently of this school-wide setting. That resolver uses the teacher's official timetable and roster without migrating grades, creating mappings, or changing manual class tabs.
...
- Teacher-scheduled ClassPilot kiosks require both the class-model and activity capabilities even in legacy schools. Temporary Send to Kiosk overrides use the school's manual class source. Pass attribution follows the resolved assignment's source; history reads support both sources, with testing/coverage restricted to the issuer and managers. A ClassPilot kiosk pass in a legacy school does not set the school-wide canonical-write cutover marker.
...
- **Admin settings**: Allowed Domains express off-task intent; Blocked Websites are administrator-curated. Safety review, exact-URL approvals and notification delivery are in Safety Center. Legacy AI email/automatic-closure controls are removed from the active workflow.
...
- **Pub/Sub webhook**: `src/routes/mailpilot/pubsub.ts` — bearer-token auth via `MAILPILOT_PUBSUB_VERIFY_TOKEN` (legacy query string `?token=...` also supported). Acknowledge only after the history batch completes; transient processing failures return 503 for provider retry. Email alerts, shared safety cases, and the outbox commit atomically. Stable message identity prevents reviewed/closed cases from reopening on replay. Watch renewal preserves unfinished cursors; expired history requires explicit recovery, never a silent cursor jump. See `docs/MAILPILOT_DURABILITY.md`.
...
the registry. Production's explicit Terraform baseline now adopts the observed
90-table roadmap allowlist verified on September 6, 2026; its recorded runtime
order differs from the historical target order. Future additions still require
...
3. Release the Chrome extension separately from `C:\GitHub\ClassPilot`. Chrome
   ...
   live at `2.7.2` (August 27, 2026). That observation is not managed-Chromebook
...
4. Omit the one-shot RLS flag on later deploys. The 75-table target above is
   ...
   separately verified 90-table roadmap inventory. Verify the current live
...
# rolloutState=COMPLETED with desiredCount==runningCount. The API carries a scheduled
# weekday school-day floor of 3 from 05:45 to 16:00 America/New_York, so desiredCount=3
# is the correct healthy reading for most of a school day; only the worker is ever 1.
...
4. **Task not starting** — If the new task fails to start after a service update, ECS rolls back automatically. Check CloudWatch logs for the failed task. Common causes: missing env vars, bad image, port mismatch. Rollback is explicit now, and must target **the family that was actually serving** — production runs `schoolpilot-production-api-emergency`, so `update-service --task-definition schoolpilot-production-api:<previousRev>` is a family switch to a revision that never served, not a rollback. `deploy.sh` captures the live definition into `PRODUCTION_ROLLBACK_API_TASK_DEFINITION` before mutating anything; by hand, read the current ARN from `describe-services` first and roll back within that family, or `schoolpilot-production-scheduler-worker:<previousRev>` for the worker.
