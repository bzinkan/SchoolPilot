...
During an applied testing day, the dashboard automatically shows the signed-in supervisor's active testing students in **Claimed**, even if a regular class overlaps. At the end of testing it returns to **Class** and the current session started by the school schedule, including applied custom times. For example: Homeroom → 9:00–10:45 testing → the class scheduled afterward. The Coverage badge may count school-wide groups for an administrator; automatic selection includes only that administrator's own testing students. A deliberately selected view or Observe mode stays selected until the dashboard is reopened. Automatic view selection never claims, releases, or changes students' assignments.
