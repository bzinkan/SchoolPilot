...
- **Semantic RLS registry**: `src/config/rlsRegistry.json` is the machine-readable inventory and rollout-request authority. It preserves the exact observed 72-table production snapshot from August 19, 2026 and separately defines the 75-table SchoolPilot 2.7.0 post-expand target. Production's 90-table roadmap inventory was verified on September 6, 2026; `infra/production.tfvars` adopts its exact observed CSV order while the generic default remains at the separately staged 75-table target. Compare production and the roadmap target semantically; never rewrite historical inventory order or hashes to match a later observation. `classpilot_active_hands` remains part of the full inventory but is deliberately excluded from the four-table FAB re-admission bundle.
