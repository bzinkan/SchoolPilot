...
- **Chat durability**: student and teacher FAB writes are transaction-authorized and text is capped at 500 characters. Teacher replies use durable delivery/outbox rows. Chat acknowledgements use their own exact-bound WebSocket/HTTP `/api/classpilot/device/chat-acks` protocol and `chat-message-ack-receipt` keyed by `messageId`; never mix chat ACKs with command ACKs.
