...
class FakeIndexClient {
  ...
  async query(text: string, params?: unknown[]): Promise<{ rows: unknown[] }> {
    ...
    if (
      ...
      normalized.includes(HEARTBEAT_HISTORY_BUILD_INDEX_NAME)
    ) {
      const created = exactState(HEARTBEAT_HISTORY_BUILD_INDEX_NAME);
      if (this.makeCreatedBuildInvalid) {
        created.indisvalid = false;
      }
      this.states.set(HEARTBEAT_HISTORY_BUILD_INDEX_NAME, created);
      return { rows: [] };
    }

    if (normalized.startsWith("DROP INDEX CONCURRENTLY")) {
      if (normalized.includes(HEARTBEAT_HISTORY_BUILD_INDEX_NAME)) {
    ...
    if (normalized.startsWith("ALTER INDEX")) {
      const build = this.states.get(HEARTBEAT_HISTORY_BUILD_INDEX_NAME);
...
describe("heartbeat teacher-tile history index contract", () => {
  ...
  it("uses pg_index plus pg_get_indexdef and has no blocking build fallback", () => {
    ...
    assert.doesNotMatch(
      source,
      /CREATE INDEX(?: IF NOT EXISTS)? \$\{HEARTBEAT_HISTORY_BUILD_INDEX_NAME\}/,
    ...
    assert.match(
      source,
      /ALTER INDEX public\.\$\{HEARTBEAT_HISTORY_BUILD_INDEX_NAME\}/
    );
  });
