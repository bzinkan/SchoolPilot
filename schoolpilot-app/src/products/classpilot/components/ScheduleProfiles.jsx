...
export default function ScheduleProfiles({ blockedByAdvancedDraft = false, onBusyChange }) {
  ...
  return <Card data-testid="schedule-profiles"><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div className="space-y-1.5"><CardTitle>Schedule Profiles</CardTitle><CardDescription>Save reusable class times and testing blocks, then preview them on the dates you choose.</CardDescription></div><Button disabled={!data || busy || blockedByAdvancedDraft} onClick={() => open('edit')}><Plus className="mr-2 h-4 w-4" />Create Schedule Profile</Button></div></CardHeader><CardContent className="space-y-5">
    ...
    {blockedByAdvancedDraft && <p className="text-sm text-amber-700 dark:text-amber-300">Save or discard the advanced schedule or calendar draft before changing profiles or their applications.</p>}
