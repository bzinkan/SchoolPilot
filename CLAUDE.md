...
- **Protocol v3 repaired-client gate**: `scopedAuthorityChecksV1` is the required parent marker for all nine dependent repaired capabilities: `authBoundTelemetryV1`, `exactBindingAckV2`, `exactTabCloseV2`, `studentChatIdempotencyV1`, `screenshotObservationLeaseV1`, `screenshotTrackingWindowLeaseV1`, `safetyEvidenceCaptureV1`, `liveViewIceServersV1`, and `kioskLaunchTicketV2`. The server must refuse every dependent unless the marker is advertised and accepted; never infer repair status from version text. Protocol-3 selected-tab controls require accepted `exactTabCloseV2`; protocol-2 clients retain negotiated V1 behavior. `kioskLaunchTicketV1` stays off. The 2.7.1 activation and rollback contract remains in `docs/CLASSPILOT_2_7_1_RELEASE.md`; the additive 2.7.3 screenshot contract is in `docs/CLASSPILOT_2_7_3_RELIABILITY_HARDENING.md`.
...
- `CLASSPILOT_PROTOCOL_V3_ENABLED` plus the per-capability flags are kill switches. `CLASSPILOT_CAPABILITY_ROLLOUTS_JSON` optionally supplies fail-closed `off`, `observe`, `canary`, or `on` policy per capability, with optional `schoolIds` and a deterministic school-level `canaryPercent`. Capability acceptance still requires a protocol-v3 client advertisement from the current exact binding, and all nine dependent repaired capabilities require accepted `scopedAuthorityChecksV1`. The existing repaired capabilities remain governed by `docs/CLASSPILOT_2_7_1_RELEASE.md`; additive `screenshotTrackingWindowLeaseV1` remains independently off until the 2.7.3 pilot described in `docs/CLASSPILOT_2_7_3_RELIABILITY_HARDENING.md`. Superseded `kioskLaunchTicketV1` stays `off`. The approved synthetic-only exception records `validationLevel=synthetic_only` and `managedValidation=waived_not_passed`; it must never be described as managed-Chromebook validation.
