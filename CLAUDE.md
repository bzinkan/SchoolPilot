...
- **Teacher FAB context**: render it for the teacher's owned active class **and for a supervision context they hold** — a scheduled block or an ad hoc claim both open in the Class workspace with `capabilities.fab === true`. Label the context explicitly and close its panels when context changes. It remains hidden in Available and Observe-other modes.
