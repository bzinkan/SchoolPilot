...
The axis uses the exact configured start and end of the school's enabled, valid daytime **Monitoring Hours** in the school timezone. Temporary class or testing edits never expand it. Disabled or absent configuration uses a clearly labeled **08:00–16:00 display default**. If configured hours are unavailable, invalid or overnight, the planner shows the editable **List** until they can be read or corrected. Out-of-range class and testing times remain displayed, editable and labeled as outside school hours; bars clip to the visible axis without altering schedule values. This changes the display only and adds no scheduling, save or apply restrictions; existing server validation remains in effect.
...
### Delete a past application from history

Use **Delete from history** only after every application date is before today in the school's timezone and all supervision for that application has ended. An application with today's or future dates, or ongoing supervision, cannot be hidden. Review the accessible confirmation naming the application before confirming. It explains that the action removes only the overview history entry: the reusable profile, full application snapshots, activity reports and audit history remain retained. This action does not cancel schedules or end supervision.

Unknown or unsettled testing completion keeps the action unavailable; use **Refresh status** after resolving supervision. If scheduling changes while the confirmation is open, close it, refresh and reopen it. A successful removal followed by a failed read shows **History entry deleted; list refresh unavailable**; **Retry schedule list refresh** repeats only the read.

### Delete a reusable profile

In **Saved profiles**, open **⋯ → Delete profile** on the row and confirm the named profile. Deletion removes it from the reusable list and frees one saved-profile slot. It cannot be undone. **Applied schedules will remain scheduled. Deleting this profile does not cancel testing.** Upcoming and active applications continue; earlier and cancelled applications keep their original snapshot names, dates and histories. Their visible entries remain in **Applied dates** or **Earlier and cancelled applications**, with **Saved profile deleted** shown beside the saved application name.
...
For **Delete from history**, deploy the compatible API and scheduler worker before the frontend. No database migration is required. After the first history removal, any backend rollback must continue to read and preserve the optional application metadata `historyHiddenAt` and `historyHiddenBy`; older strict application parsers reject these fields. Keep a compatible API and worker or roll forward. Removing the metadata is not a rollback procedure.
