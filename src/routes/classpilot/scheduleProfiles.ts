...
router.delete("/applications/:id/history", async (req, res, next) => {
  try {
    if (!req.body || typeof req.body !== "object" || Array.isArray(req.body) || Object.keys(req.body).some(key => key !== "revision")) {
      throw schedulingError("Provide only the current scheduling revision.", "SCHEDULE_PROFILE_INVALID", 400);
    }
    const result = await hideScheduleProfileApplicationHistory({ schoolId: res.locals.schoolId!, actorId: req.authUser!.id,
      applicationId: String(req.params.id), revision: req.body.revision });
    await broadcastClasspilotScheduleChangeUpdate({ schoolId: res.locals.schoolId!, revision: result.revision }).catch(() => undefined);
    res.json(result);
  } catch (error) { next(error); }
});
