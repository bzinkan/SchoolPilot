...
- **Coverage command scoping**: Claimed-student actions go through `POST /api/classpilot/coverage/contexts/:id/commands`, grouped by immutable supervision context. The coverage allowlist is `open-tab`, `close-tabs`, `lock-screen`, `unlock-screen`, `teacher-message`, `apply-flight-path`, and `apply-block-list`; Manual/ad hoc supervision retains this allowlist. With the scheduled-classroom rollout enabled, verified scheduled contexts use the typed canonical classroom route described below for expanded tools; the selected UI tab never establishes authority. Cross-context UI dispatch preserves partial outcomes and never converts one failed context into a class-wide fallback.
...
- **Claimed-coverage route**: `POST /api/classpilot/coverage/contexts/:contextId/commands` accepts only the coverage allowlist documented under School Isolation. Manual/ad hoc coverage retains its existing capabilities. Verified scheduled supervision uses the separate additive canonical classroom contract, not an enlarged ad hoc allowlist.
- **Scheduled classroom authority (gated)**: `CLASSPILOT_SCHEDULED_CLASSROOM_MODE=off|on` defaults off; optional `CLASSPILOT_SCHEDULED_CLASSROOM_SCHOOL_IDS` restricts rollout. `GET /api/classpilot/dashboard-activity` is the personal scheduled workspace resolver. Class includes scheduled testing/coverage; Claimed's `category=ad_hoc` excludes them only under the rollout. `/commands` accepts exactly one `teachingSessionId` or `supervisionContextId`; only server-owned application/coverage metadata can authorize expanded context tools. `scheduledClassroomV1` plus `scopedAuthorityChecksV1` is required on the extension. Six classroom tables support exactly one same-school parent. Context settings, chat, hands, polls, Live View, observations and screenshots use that exact authority, with stale ownership rejected. No synthetic teaching sessions for testing. See `docs/CLASSPILOT_AUTOMATIC_CLASS.md` for rollout order and rollback constraints.
- **Scheduled boundary worker**: `classpilot_school_schedules` carries an indexed deadline, generation and short lease, independent of the scheduling document revision. The one-second bounded worker reconciles lifecycle transitions; transaction wake triggers preserve concurrent edits. Keep report/backfill discovery outside this lane. Realtime plus known-boundary timers and reconnect checks target five seconds for healthy connected clients. Reporting-only classes remain awaiting live supervision, never active control.
