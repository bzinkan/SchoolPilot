  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">Create Homerooms</h2>
          <p className="text-gray-500 dark:text-slate-400 text-sm">Add homeroom classes. Students will be assigned in the next tab.</p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border dark:border-slate-700 p-4">
          <form onSubmit={handleSubmit} className="flex items-end gap-4">
            <div className="flex-1">
              <label className="block text-sm font-medium text-gray-700 dark:text-slate-200 mb-1">Teacher *</label>
              {teachers.length > 0 ? (
                <select value={teacherId} onChange={(e) => setTeacherId(e.target.value)}
                  className="w-full px-3 py-2 border dark:border-slate-600 dark:bg-slate-800 dark:text-white rounded-lg" autoFocus>
                ...
                <div>
                  <input type="text" value={teacherName} onChange={(e) => setTeacherName(e.target.value)}
                    placeholder="Teacher name" className="w-full px-3 py-2 border dark:border-slate-600 dark:bg-slate-800 dark:text-white rounded-lg" autoFocus />
            ...
            <div>
              ...
              <select value={grade} onChange={(e) => setGrade(e.target.value)} className="px-3 py-2 border dark:border-slate-600 dark:bg-slate-800 dark:text-white rounded-lg">
            ...
            <button type="button" onClick={() => setShowForm(false)} className="px-4 py-2 border dark:border-slate-700 rounded-lg text-sm hover:bg-gray-50 dark:hover:bg-slate-800">
              Cancel
            </button>
          </form>
        </div>
      ...
      {homerooms.length > 0 ? (
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border dark:border-slate-700 p-12 text-center">
          <School className="w-12 h-12 text-gray-300 dark:text-slate-600 mx-auto mb-3" />
          <p className="text-gray-500 dark:text-slate-400">No homerooms created yet</p>
          <p className="text-sm text-gray-400 dark:text-slate-500">Click a grade button above to add your first homeroom</p>
        </div>
      )}
    </div>
  );
