...
| CLAIM-003 | AI Transparency, Subprocessors | AI data sent to subprocessors is limited and disclosed. | Engineering | AI data-flow inventory, audit events, redaction tests, updated public pages; private paperwork-import provider/account and synthetic quality review before enablement | Needs remediation | Verify each enabled flow; new conditional import disclosures and tests are not production or extraction-accuracy evidence |
...
| CLAIM-006 | WISP, HECVAT | Privileged access is reviewed; in-app MFA is deferred. | Security & Privacy Officer | Privileged access evidence packet, private user/role export, quarterly access review approval, MFA deferral risk acceptance, MFA export when implemented | Needs remediation | Keep MFA claims partial/deferred until access review and MFA deferral decisions are approved; do not claim MFA is live |
